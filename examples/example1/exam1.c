...
#define FUNC_BDT	"Simple example"
...
/*--------------------------------------------------------------------
 ...
 +--------------------------------------------------------------------*/
static char *fichImagen  = "EXAM1.ibd";
static int   fdFBD	 = -1;
 ...
 *====================================================================*/
void EXAM1_Directorio(char *directorio)
{
	char *nuevoPath;
...
/*--------------------------------------------------------------------
 ...
 +--------------------------------------------------------------------*/
nuevoPath = (char *) malloc(strlen(directorio) + sizeof("EXAM1.ibd") + 1);
if (nuevoPath == NULL)
    ERR_SistOper(ERR_FATAL, FUNC_BDT, "EXAM1_Directorio", "malloc",
		 "(%d bytes) path de la base", lonMemoria);

/*--------------------------------------------------------------------
 | Se compone el nuevo path.
 +--------------------------------------------------------------------*/
sprintf(nuevoPath, "%s/EXAM1.ibd", directorio);
 ...
 *====================================================================*/
void EXAM1_Resize(int tabla, int dimension)
{
switch(tabla) {
    case TBL_TABLE:
	/*------------------------------------------------------------
	...
	if (dimension > MAX_TABLE)
	    ERR_Aplicacion(ERR_NO_FATAL, FUNC_BDT, "EXAM1_Resize",
 ...
 *====================================================================*/
void EXAM1_Overflow(int tabla, int desbord)
{
switch(tabla) {
    case TBL_TABLE:
	ovfTable = desbord;
...
if (pMemoria == NULL) {
    ERR_SistOper(ERR_NO_FATAL, FUNC_BDT, "EXAM1_Inicializar", "malloc",
...
if (fichAbierto)
    if (close(fdFBD) == -1) {
	ERR_SistOper(ERR_NO_FATAL, FUNC_BDT, "EXAM1_Close", "close",
			"file \"%s\"", fichImagen);
	return -1;
	}
return 0;
 ...
 *====================================================================*/
int EXAM1_NumRecords(int tabla)
{
switch(tabla) {
    case TBL_TABLE:
	return *numTable;
 ...
 *====================================================================*/
int EXAM1_InsTable(TypeTABLE *pTable)
{
...
if (BuscaElemento(pTable, clvSequenceTable,
    ...
    ERR_Aplicacion(ERR_NO_FATAL, FUNC_BDT, "EXAM1_InsTable",
 ...
 *	-1: si se produce un error.
 *====================================================================*/
int EXAM1_DelTable(TypeTABLE *pTable)
{
	char elemCod[LON_TABLE];
	int  posicMem;
	int  posicFic;
...
if (indElem == -1) {
    ERR_Aplicacion(ERR_NO_FATAL, FUNC_BDT, "EXAM1_EliTable",
 ...
 *	-1: si se produce un error.
 *====================================================================*/
int EXAM1_ModTable(TypeTABLE *pTable)
{
	char elemCod[LON_TABLE];
	int  posicMem;
	int  posicFic;
...
indElem = BuscaElemento(pTable, clvSequenceTable,
		*numTable, (PF_COMPAR)CmpSequenceTable);
if (indElem == -1) {
    ERR_Aplicacion(ERR_NO_FATAL, FUNC_BDT, "EXAM1_ModTable",
		   "key SEQUENCE of TABLE not exists");
    return -1;
    }
 ...
 *	Busca un elemento
 *====================================================================*/
int EXAM1_TableBySequence(TypeTABLE *pTable,
				int            mykey)
{
	int  indElem;
...
if (pTable == NULL) {
    ERR_SistOper(ERR_NO_FATAL, FUNC_BDT, "EXAM1_VolTable",
...
if (access(fichImagen, F_OK) == -1) {
    ERR_Aplicacion(ERR_NO_FATAL, FUNC_BDT, "EXAM1: IniInformacion",
...
if (fdFBD == -1) {
    ERR_SistOper(ERR_NO_FATAL, FUNC_BDT, "EXAM1: IniInformacion", "open",
...
if (bufElems == NULL)
    ERR_SistOper(ERR_FATAL, FUNC_BDT, "EXAM1 IniInformacion", "malloc",
...
if (lseek(fdFBD, POS_TABLE, SEEK_SET) == -1) {
    ERR_SistOper(ERR_NO_FATAL, FUNC_BDT, "EXAM1: IniInformacion", "lseek",
...
if (read(fdFBD, bufElems, BUF_TABLE) != BUF_TABLE) {
    ERR_SistOper(ERR_NO_FATAL, FUNC_BDT, "EXAM1: IniInformacion", "read",
...
if (! fichAbierto)
    if (close(fdFBD) == -1) {
	ERR_SistOper(ERR_NO_FATAL, FUNC_BDT, "EXAM1: IniInformacion", "close",
		     "file \"%s\"", fichImagen);
	return -1;
	}
...
TPC_CodChar(pTable->text, 50, elem + 5);
}
...
TPC_DecChar(pTable->text, 50, elem + 5);
}
...
ERR_Aplicacion(ERR_NO_FATAL, FUNC_BDT, "EXAM1: KeyTable",
...
if (! fichAbierto) {
    fdFBD = open(fichImagen, O_RDWR | O_EXCL);
    if (fdFBD == -1) {
	ERR_SistOper(ERR_NO_FATAL, FUNC_BDT, "EXAM1: ActElemento", "open",
		 "file \"%s\" (for read/write))", fichImagen);
	return -1;
	}
    }
...
if (lseek(fdFBD, (off_t)posFic, SEEK_SET) == -1) {
    ERR_SistOper(ERR_NO_FATAL, FUNC_BDT, "EXAM1: ActElemento", "lseek",
...
if (write(fdFBD, (char *)eleFic, lonFic) != lonFic) {
    ERR_SistOper(ERR_NO_FATAL, FUNC_BDT, "EXAM1: ActElemento", "write",
...
if (! fichAbierto)
    if (close(fdFBD) == -1) {
	ERR_SistOper(ERR_NO_FATAL, FUNC_BDT, "EXAM1: ActElemento", "close",
		     "file \"%s\"", fichImagen);
	return -1;
	}
